...
## Iteration 32（仓库级 Codex 执行边界固化，2026-03-12）

### 决策
- 在仓库根新增 `AGENTS.md` 作为 Codex/自动化协作的项目级单一说明。
- 将“跨安全边界业务交换网关”协议与工程边界显式化：
  - SIP：JSON body 承载完整业务字段，header 仅镜像索引字段；
  - RTP：二进制定长主头 + 可选 TLV 扩展段；
  - HTTP：仅允许基于 `api_code` 的受控模板执行，不提供任意透传；
  - 签名：首版 HMAC-SHA256，并预留国密算法升级位。
- 将 Rust 主干红线同步到仓库级规则：panic-free、unsafe 受控、`--locked`/lockfile 一致性、最小公开面与集中指标命名。

### 依据
- 现有规则分散在会话提示与文档上下文中，易在跨会话协作时漂移；仓库级入口可降低执行偏差。
- 通过统一入口文档可确保“协议边界 + 工程红线 + 文档同步要求”同源演进，减少重复沟通成本。

### 不变项
- 不改变 driver scheduling kernel 语义与 `install_channel() -> sync_interest(chan_id)` 契约基线。
- 不引入新的运行时功能路径，仅做执行规范与文档边界固化。
