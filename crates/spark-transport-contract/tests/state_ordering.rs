...
fn writability_change_and_interest_follow_watermarks_in_order() {
    ...
    let io: Box<dyn spark_transport::async_bridge::contract::DynChannel> = Box::new(ScriptedIo::new());
    let flush = FlushPolicy::default().budget(64 * 1024);
    let mut st = ChannelState::new(1, io, 8 * 1024, 4 * 1024, flush, sink_erased);

    // enqueue > high watermark => BackpressureEnter, READ paused, interest WRITE only.
    st.enqueue_outbound(OutboundFrame::from_bytes(Bytes::from(vec![0u8; 16 * 1024])));
    ...
    assert!(matches!(wc1, spark_transport::async_bridge::contract::WritabilityChange::BecameUnwritable { .. }));
    ...
    assert!(matches!(wc2, spark_transport::async_bridge::contract::WritabilityChange::BecameWritable { .. }));
    ...
    let enter = names.iter().position(|n| *n == ev_names::BACKPRESSURE_ENTER).unwrap();
    let exit = names.iter().position(|n| *n == ev_names::BACKPRESSURE_EXIT).unwrap();
    assert!(enter < exit, "expected BackpressureEnter before BackpressureExit: {:?}", names);
}
...
fn draining_timeout_emits_in_order_and_requests_close() {
    ...
    assert!(st.desired_interest().is_empty(), "expected empty interest when draining without backlog");

    std::thread::sleep(Duration::from_millis(20));
    st.poll_draining(0);
    assert!(st.is_close_requested(), "expected close requested after draining timeout");

    let evs = sink.take();
    let names: Vec<&'static str> = evs.iter().map(|e| e.name).collect();
    let enter = names.iter().position(|n| *n == ev_names::DRAINING_ENTER).unwrap();
    let timeout = names.iter().position(|n| *n == ev_names::DRAINING_TIMEOUT).unwrap();
    assert!(enter < timeout, "expected DrainingEnter before DrainingTimeout: {:?}", names);
}
