...
impl ServerBuilder<NoPipeline> {
    ...
    pub fn udp_connected(mut self, remote: SocketAddr) -> Self {
        self.transport = Transport::UdpConnected { remote };
        self
    }
    ...
    {
        ServerBuilder { host: self.host.pipeline(build), transport: self.transport }
    }
}
...
impl<S> ServerBuilder<S> {
    ...
    /// Forward mgmt route configuration to the underlying host builder.
    pub fn map_get<F, Fut>(mut self, path: impl Into<Box<str>>, route_id: impl Into<Box<str>>, handler: F) -> Self
    where
    ...
    pub fn map_post<F, Fut>(mut self, path: impl Into<Box<str>>, route_id: impl Into<Box<str>>, handler: F) -> Self
    where
        F: Fn(spark_host::router::MgmtRequest) -> Fut + Send + Sync + 'static,
        Fut: std::future::Future<Output = spark_host::router::MgmtResponse> + Send + 'static,
    {
    ...
    S: Service<Bytes, Response = Option<Bytes>, Error = KernelError> + Send + Sync + 'static,
{
    pub fn build(self) -> core::result::Result<Server<S>, KernelError> {
        ...
        Ok(Server { spec, transport: self.transport })
    }
}
